...
-   Decide how to work with missing values,\
-   Decide how to work with outliers,\
...
```{r echo=FALSE}
df.train |> summarise(n=n(), .by=churn) |> 
  mutate(pct = n/sum(n)) |> 
  gt(rowname_col="stub", locale="pt") |> sub_missing() |>
  fmt_percent(pct, decimals=0) |> fmt_integer(n) |> 
  tab_options(
  ...
  tab_header(title = md("**Table: Churn status**"))
```
...
PlotCategorical <- function(df, cat_var, title) {
  ...
  df.plt <- df |> 
    ...
    mutate(churn = if_else(churn=="Yes", "Cancelled", "Renewed"))
...
PlotCategorical(df.train, gender, "Gender")
```
...
```{r echo=FALSE, fig.width=5, fig.height=5}
PlotCategorical(df.train, senior_citizen, "IS Senior Citizen?")
```
...
```{r}
summarytools::descr(df.train)
```

\

```{r echo=FALSE, fig.width=6.5, fig.asp=.4}
df.train |> select(is.numeric) |> 
  ...
  mutate(Mean = mean(value), 
         ...
         .by=c(name)) |>
  ggplot(aes(value)) +
  geom_histogram(bins=40, color="white", fill="slategray", alpha=.7) +
  ...
  labs(title="Histograms of numerical variables", x=NULL, y="Frequency")
```

\
For numeric variables, I would expect the distributions of each variable against the outcome variable to be different from each other (churn x no churn) otherwise the explanatory variable would add little information about the outcome.

```{r echo=FALSE, fig.asp=.7, fig.width=6.5}
df.train |> select(is.numeric, churn) |> 
  mutate(churn = if_else(churn=="Yes", "Customers cancelled (churn=Yes)", 
                         "Customers renewed (churn=No)")) |> 
...
options(scipen=999)
inner_join(
  summarytools::descr(filter(df.train, churn=="Yes")) |> 
    as.data.frame() |> rownames_to_column("id") |> pivot_longer(-id, values_to="churn"),
  summarytools::descr(filter(df.train, churn=="No")) |> 
    as.data.frame() |> rownames_to_column("id") |> pivot_longer(-id, values_to="churn"),
